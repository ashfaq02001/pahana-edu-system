...
public class BillController extends HttpServlet {
    ...
    private void generateBill(HttpServletRequest request, HttpServletResponse response)
        ...
        if (itemIds != null) {
            ...
            for (int i = 0; i < itemIds.length; i++) {
                System.out.println("Item " + i + ": ID=" + itemIds[i] + ", Qty=" + (quantities != null && i < quantities.length ? quantities[i] : "null"));
            }
        } else {
    ...
    private void printReceipt(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            summaryTable.addCell(new PdfPCell(new Phrase("$" + String.format("%.2f", bill.getTotalAmount()), boldFont)));
